...
<?php include __DIR__ . '/../Autre/menu_haut.php'; ?>
<br />
<br/>
<br />
...
<aside id="menu">
    ...
    <div id="films-container" class="films-container">
        ...
        $filmController2 = new FilmController2($connexion);

        $resultatFilms2 = $filmController2->displayFilms2();
        $resultatImage2 = $filmController2->displayDefaultImage2();

        if ($resultatFilms2->rowCount() > 0) {
            while ($row = $resultatFilms2->fetch(PDO::FETCH_ASSOC)) {
                $afficheFilm = base64_encode($row['affiches']);
                echo "<img src='data:image/jpeg;base64,$afficheFilm' alt='Affiche du film'>";
            }
        } else {
            if ($resultatImage2->rowCount() > 0) {
                $rowImage = $resultatImage2->fetch(PDO::FETCH_ASSOC);;
                $imageParDefaut2 = base64_encode($rowImage['image']);
                echo "<img src='data:image/jpeg;base64,$imageParDefaut2' alt='Image par défaut'>";
            } else {
